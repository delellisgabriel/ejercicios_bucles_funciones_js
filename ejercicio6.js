...
const starWarsMoviesByDecade = {}

for (const starWarsMovie of starWarsMovies) {
  const decade = Math.floor(starWarsMovie.releaseYear / 10) * 10
  if (!starWarsMoviesByDecade[decade]) {
    starWarsMoviesByDecade[decade] = []
  }
  starWarsMoviesByDecade[decade].push(starWarsMovie.title)
}

console.log(starWarsMoviesByDecade)
